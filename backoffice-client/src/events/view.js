...
export default withRouter(
  class ViewEvent extends React.Component {
    ...
    componentDidMount() {
      base
        .fetch(`opportunities/${this.props.match.params.id}`, {
          context: this
        })
        .then(data => {
          this.setState({
    ...
    render() {
      ...
      return (
        <div>
          ...
          <div>
            <strong>Start:</strong>{" "}
            {moment(event.start).format("dddd, MMMM Do YYYY, h:mm:ss a")}
          </div>
          <div>
            <strong>End:</strong>{" "}
            {moment(event.finish).format("dddd, MMMM Do YYYY, h:mm:ss a")}
          </div>
          <div>
